...
for i in {0..99}; do
  ...
  echo "function vits_cf_available_for_beginner_field_callback_function_$i()" >>$FILE
  echo "{" >>$FILE
  echo "    vits_cf_available_for_beginner_field_callback_function($i);" >>$FILE
  echo "}" >>$FILE
  echo "" >>$FILE

  echo "function vits_cf_available_for_intermediate_field_callback_function_$i()" >>$FILE
  echo "{" >>$FILE
  echo "    vits_cf_available_for_intermediate_field_callback_function($i);" >>$FILE
  echo "}" >>$FILE
  echo "" >>$FILE

  echo "function vits_cf_available_for_nerd_field_callback_function_$i()" >>$FILE
  echo "{" >>$FILE
  echo "    vits_cf_available_for_nerd_field_callback_function($i);" >>$FILE
  echo "}" >>$FILE
  echo "" >>$FILE
